...
$route = function ($handler) {
    try {
        ...
        if (in_array($doing, ['importAdd', 'importStats', 'importProcess', 'importEdit', 'importDelete']) && CHV\Login::isAdmin() == false) {
        ...
        switch ($doing) {
            ...
            case 'upload': // EX 100
                ...
                $type = $_REQUEST['type'];
                $owner_id = !empty($_REQUEST['owner']) ? CHV\decodeID($_REQUEST['owner']) : $logged_user['id'] ?? null;

                if (isset($_REQUEST['what']) && in_array($_REQUEST['what'], ['avatar', 'background'])) {
                    if (!$logged_user) {
                ...
                if (!$handler::getCond('content_manager') && CHV\getSetting('akismet')) {
            ...
            case 'list': // EX 200
                ...
                if (isset($_REQUEST['params_hidden']) && is_array($_REQUEST['params_hidden'])) {
                    $params_hidden = [];
                ...
                if (array_key_exists('approved', $_REQUEST)) {
                    if (CHV\Login::isAdmin() || $logged_user['is_manager']) {
                ...
                switch ($type) {
                    ...
                    case 'image':
                        ...
                        if (
                            ...
                            && !(CHV\Login::isAdmin() || $logged_user['is_manager'])
                        ) {
                    ...
                    case 'album':
                        ...
                        $album_edited = CHV\Album::getSingle($editing['album_id'] ?? $id);

                        if (!isset($album_edited)) {
            ...
            case 'create-album':
                ...
                if (!$handler::getCond('content_manager') && CHV\getSetting('akismet') && $album['new']) {
                ...
                if (!empty($ids) && is_array($ids)) {
                    ...
                    if ($type == 'images') {
                        ...
                        CHV\Album::moveContents($albums_ids, $album_id);
                    }
                }
            ...
            case 'delete':
                ...
                $owner_id = isset($_REQUEST['owner']) ? CHV\decodeID($_REQUEST['owner']) : $logged_user['id'];

                $multiple = ($_REQUEST['multiple'] ?? null) == 'true';
                $single = ($_REQUEST['single'] ?? null) == 'true';
                if (!$multiple) {
                    $single = true;
                }
            ...
            case 'disconnect': // admin only
                ...
                if (!CHV\Login::isAdmin() and $user_id and $user_id !== $logged_user['id']) {
                    throw new Exception('Invalid request', 403);
                }
            ...
            case 'album-cover-unset':
                ...
                $image_pub_id = $_POST[$doing]['image_id'];
                $album_pub_id = $_POST[$doing]['album_id'];
                $image_id = CHV\decodeID($image_pub_id);
                $album_id = CHV\decodeID($album_pub_id);
                $image = CHV\Image::getSingle($image_id, false, true);
                $album = CHV\Album::getSingle($album_id, false, true);
                if ($image['album']['id'] !== $album['id']) {
                    throw new Exception("Image doesn't belong to this album", 100);
                }
                $isLoggedOwner = $image['user']['id'] !== $logged_user['id'] || $album['user']['id'] !== $logged_user['id'];
                if (!$handler::getCond('content_manager') && $isLoggedOwner) {
                    throw new Exception('Invalid content owner request', 101);
                }
                CHV\Album::update($album_id, ['cover_id' => $doing == 'album-cover-unset' ? null : $image_id]);
                $json_array['success'] = [
            ...
            case 'notifications':
                ...
                if ($notifications) {
                    ...
                    $avatar_src_tpl = [
                        0 => '<span class="user-image default-user-image"><span class="icon fas fa-meh"></span></span>',
            ...
            case 'approve':
                ...
                if (($_REQUEST['multiple'] ?? null) == 'true') {
        ...
        $errorId = G\exception_to_error($e, false);
        $message = "<b>Something went wrong:</b> ⚠ errorId #$errorId";
        $debug_level = (int) (G\get_app_setting('debug_level') ?? 1);
        if(in_array($debug_level, [2, 3])) {
            $message .= "<p class='growl-debug-status'>Printed debug is enabled because <code class='font-weight-bold'>debug_level=$debug_level</code></p>";
            $message .= '<div class="growl-debug-message">';
            $message .= "<p>> " . safe_html($e->getMessage()) . "</p>";
            $message .= "<pre class='growl-debug-trace'><code>" . $e->getTraceAsString() . '</code></pre>';
            $message .= '</div>';
        }
        $json_array = [
            'status_code' => 500,
