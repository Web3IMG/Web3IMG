...
$route = function ($handler) {
    ...
    $icons = [
        ...
        'password'              => 'fas fa-key',
        'linked-accounts'     => 'fas fa-people-arrows',
        ...
        if (!$is_error) {
            ...
            if (in_array($doing, [null, 'account', 'profile'])) {
                ...
                if ($is_changed) {
                    ...
                    if ($logged_user['is_admin'] && isset($_POST['role'])) {
                        $is_manager = 0;
                        $is_admin = 0;
                        switch ($_POST['role']) {
                            case 'manager':
    ...
    if ($doing == 'linked-accounts') {
        ...
        $logins = CHV\Login::get(['user_id' => $user['id']]);
        $available_connections = CHV\Login::getSocialServices(['get' => 'enabled']);

        $has_password = false;

        $connections = [];
        foreach ($logins as $pos => $login) {
            if ($login['type'] == 'password') {
                $has_password = true;
            }
            if (!array_key_exists($login['type'], $available_connections)) {
                continue;
            }
            $connections[$login['type']] = $login;
            $connections[$login['type']]['type_label'] = $available_connections[$login['type']];
        }
