...
function echoDatabaseEnv(string $env, string $post, string $default): void {
    echo 'placeholder="' . $default . '" ';
    $getEnv = getenv($env);
    echo 'value="';
    if($getEnv !== false) {
        echo getenv($env) .'" readonly';
    } else {
        echo ($safe_post[$post] ?? '') . '"';
    }
}
...
<form method="post" class="c9">
	<div class="input-label">
		...
		<input type="text" name="db_host" id="db_host" class="text-input" <?php echoDatabaseEnv('CHEVERETO_DB_HOST', 'db_host', 'localhost'); ?> title="Database server host (default: localhost)" rel="tooltip" data-tipTip="right" required>
	</div>
	<div class="input-label">
		<label for="db_name">Database name</label>
		<input type="text" name="db_name" id="db_name" class="text-input" <?php echoDatabaseEnv('CHEVERETO_DB_NAME', 'db_name', 'Database name'); ?> title="Name of the database where you want to install Chevereto" rel="tooltip" data-tipTip="right" required>
	</div>
	<div class="input-label">
		<label for="db_user">Database user</label>
		<input type="text" name="db_user" id="db_user" class="text-input" <?php echoDatabaseEnv('CHEVERETO_DB_USER', 'db_user', 'Database user'); ?> title="User with access to the above database" rel="tooltip" data-tipTip="right" required>
	</div>
	<div class="input-label">
		<label for="db_pass">Database user password</label>
		<input type="text" name="db_pass" id="db_pass" class="text-input" <?php echoDatabaseEnv('CHEVERETO_DB_PASS', 'db_pass', 'Database user password'); ?> title="Password of the above user" rel="tooltip" data-tipTip="right">
	</div>
	<div class="input-label">
		<label for="db_table_prefix">Database table prefix</label>
		<input type="text" name="db_table_prefix" id="db_table_prefix" class="text-input" <?php echoDatabaseEnv('CHEVERETO_DB_TABLE_PREFIX', 'db_table_prefix', 'chv_'); ?>  title="Database table prefix. Use chv_ if you don't need this" rel="tooltip" data-tipTip="right">
	</div>
