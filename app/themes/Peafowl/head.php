...
<head>
    ...
    <?php if (G\is_route('image') && function_exists('get_image') && get_image()['is_360']) { ?>
    <link rel="stylesheet" href="<?php echo CHV\Render\get_static_url(CHV_PATH_PEAFOWL . 'js/pannellum.css'); ?>">
    <script type="text/javascript" src="<?php echo CHV\Render\get_static_url(CHV_PATH_PEAFOWL . 'js/pannellum.js'); ?>"></script>
    <?php } ?>
    <?php
    if (!is_maintenance()) {
    ...
    switch (true) {
        ...
        case function_exists('get_user') and G\is_route('user'):
            $open_graph_extend = [
                ...
                'image' => isset(get_user()['avatar']) ? get_user()['avatar']['url'] : '',
            ];
            break;
        case function_exists('get_album') and G\is_route('album'):
            $open_graph_extend = [
    ...
    switch (true) {
        ...
        case function_exists('get_user') and G\is_route('user'):
            ...
            if (G\is_route('album')) {
                ...
                $twitter_card['creator'] = isset(get_user()['twitter']) ? get_user()['twitter']['username'] : '';
            }
            $list_output = function_exists('get_list') ? (get_list()->output_assoc ?? null) : null;
    ...
    <link rel="image_src" href="<?php echo get_image()['url']; ?>">
            ...
            if (CHV\getSetting('theme_custom_css_code')) {
                ?>
                <style>
                    <?php echo CHV\getSetting('theme_custom_css_code'); ?>
                </style>
            <?php
            }
            if (CHV\getSetting('theme_custom_js_code')) {
                ?>
                <script>
                    <?php echo CHV\getSetting('theme_custom_js_code'); ?>
                </script>
            <?php
            }
