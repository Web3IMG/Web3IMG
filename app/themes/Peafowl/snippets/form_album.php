...
<div class="c7 input-label">
    ...
    <input type="text" name="<?php echo $label; ?>" class="text-input" value="<?php echo $album["name"] ?? null; ?>" placeholder="<?php _se('Album name'); ?>" maxlength="<?php echo CHV\getSetting('album_name_max_length'); ?>" required>
	<?php if (CHV\Login::isLoggedUser() && !isset($GLOBALS['theme_include_args'], $GLOBALS['theme_include_args']['album-switch'])) {
        ?>
...
<div class="input-label overflow-auto">
    <div class="c7 grid-columns">
        ...
        foreach ($permissions as $k => $v) {
            echo '<option value="'.$k.'"' . (($album['privacy'] ?? null) == $k ? '  selected' : null) . '>'.$v['label'].'</option>';
        } ?>
		</select>
	</div>
</div>
	...
	<div data-combo-value="password" class="switch-combo<?php echo(($album['privacy'] ?? null) !== 'password' ? ' soft-hidden' : null); ?>">
		<div class="input-label overflow-auto">
			<div class="c7 grid-columns">
				...
				<input type="text" name="form-album-password" class="text-input" value="<?php echo $album['password'] ?? null; ?>" data-required<?php echo(($album['privacy'] ?? null) == 'password' ? ' required' : null); ?>>
			</div>
		</div>
	</div>
</div>
