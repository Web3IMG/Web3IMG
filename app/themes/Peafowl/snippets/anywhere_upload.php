...
<div id="anywhere-upload" class="upload-box upload-box--fixed upload-box--hidden queueEmpty" data-queue-size="0">
	...
	<div class="content-width">
    	...
    	<div class="upload-box-inner">
        	...
        	<div class="upload-box-heading c16 center-box">
				<div class="upload-box-status">
					<div data-group="upload">
                        ...
                        <?php
                            $you_can_add_two = CHV\getSetting('enable_uploads_url')
                                ? _s('You can also %i or %u.')
                                : _s('You can also %i.');
                            $you_can_add_three = CHV\getSetting('enable_uploads_url')
                                ? _s('You can also %i, %c or %u.')
                                : _s('You can also %i or %c.');
                        ?>
						<div class="device-mobile--hide upload-box-status-text"><?php echo strtr($you_can_add_two, [
						...
						<div class="device-nonmobile--hide upload-box-status-text"><?php strtr($you_can_add_three, [
					...
					<div data-group="upload-queue-ready" class="soft-hidden">
                        ...
                        <?php
                            $you_can_add_two = CHV\getSetting('enable_uploads_url')
                                ? _s('You can add more images from %i or %u.')
                                : _s('You can add more images from %i.');
                            $you_can_add_three = CHV\getSetting('enable_uploads_url')
                                ? _s('You can add more images from %i, %c or %u.')
                                : _s('You can add more images from %i or %c.');
                        ?>
						<div class="device-mobile--hide upload-box-status-text"><?php strtr($you_can_add_two, [
						...
						<div class="upload-box-status-text">
							<div data-group="user" class="soft-hidden">
								...
								<?php
                                    ...
                                    if (isset(CHV\Login::getUser()['album_count']) && CHV\Login::getUser()['album_count'] > 0) {
			...
			<input id="anywhere-upload-input" data-action="anywhere-upload-input"<?php if (!CHV\getSetting('guest_uploads')) {
                                ?> data-login-needed="true"<?php
                            } ?> class="hidden-visibility" type="file" accept="image/*, <?php echo '.' . implode(',.', CHV\Image::getEnabledImageFormats()); ?>" multiple>
			<input id="anywhere-upload-input-camera" data-action="anywhere-upload-input"<?php if (!CHV\getSetting('guest_uploads')) {
                                ?> data-login-needed="true"<?php
						...
						<select name="upload-album-id" id="upload-album-id" class="text-input">
                            ...
                            foreach ($user_albums as $album) {
                                $user_album_options_html[] = strtr('<option value="%id"%selected>%name</option>', [
                                            '%selected' => (function_exists('get_album') && isset(get_album()['id_encoded']) && get_album()['id_encoded']== $album['id_encoded']) ? ' selected' : null,
                                            '%id'	=> $album['id_encoded'],
      ...
      <div class="upload-box-allowed-files position-absolute">
				...
				<span><?php echo CHV\getSetting('upload_max_filesize_mb').' MB'; ?>
				<?php
                    if (CHV\getSetting('upload_max_filesize_mb_bak') != null && CHV\getSetting('upload_max_filesize_mb') != CHV\getSetting('upload_max_filesize_mb_bak')) {
                        ?>
				<span class="icon fas fa-exclamation-triangle color-red margin-left-5" rel="tooltip" data-tiptip="top" title="<?php _se('Register to get %s', G\format_bytes(G\get_bytes(CHV\getSetting('upload_max_filesize_mb_bak').'MB'))); ?>"></span>
				<?php
                    }
                ?>
				</span>
			...
			<div class="upload-box-close position-absolute">
				<a data-action="reset-upload" data-button="close-cancel"><span class="btn-icon fas fa-undo"></span><span class="btn-text"><?php _se('reset'); ?></span></a>
				<a data-action="close-upload" data-button="close-cancel"><span class="btn-icon fas fa-times"></span><span class="btn-text"><?php _se('close'); ?></span></a>
				<a data-action="cancel-upload" data-button="close-cancel"><span class="btn-icon fas fa-times"></span><span class="btn-text"><?php _se('cancel'); ?></span></a>
				<a data-action="cancel-upload-remaining" data-button="close-cancel"><span class="btn-icon fas fa-times"></span><span class="btn-text"><?php _se('cancel remaining'); ?></span></a>
			</div>
	...
	<div class="hidden">
        ...
        <?php if(CHV\getSetting('enable_uploads_url')) { ?>
	...
	<?php
        ...
        $new_album = true;
        $user_items_editor = [
            "user_albums"	=> CHV\Login::isLoggedUser() ? $user_albums : null,
            "type"			=> "albums"
        ];
    ?>
	<div data-modal="form-uploaded-create-album" class="hidden" data-is-xhr data-submit-fn="CHV.fn.submit_upload_edit" data-ajax-deferred="CHV.fn.complete_upload_edit">
		...
		<p><?php
            ...
            echo ' ';
            if (CHV\Login::isLoggedUser()) {
                _se('You can also move the content to an %a.', ['%a' => '<a class="modal-switch" data-switch="move-existing-album">' . _s('existing album') . '</a>']);
            } else {
