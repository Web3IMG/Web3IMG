...
<div class="content-width">
    ...
    <div class="c24 center-box">
        ...
        <div class="header header-tabs default-margin-bottom follow-scroll">
            <h1><?php echo get_safe_html_pre_doctitle(); ?></h1>
            <div class="hidden phone-show phablet-show">
                ...
                <div class="phone-display-inline-block phablet-display-inline-block hidden tab-menu current" data-action="tab-menu">
                    <span class="btn-icon <?php echo $current['icon'] ?? ''; ?>" data-content="tab-icon"></span>
                    <span class="btn-text" data-content="current-tab-label"><?php echo $current["label"]; ?></span><span class="icon fas fa-sort margin-left-5"></span>
                </div>
                <ul class="content-tabs phone-hide phablet-hide">
                    <?php
                    foreach (get_settings_menu() as $item) {
                        ?>
                        <li<?php if ($item["current"]) {
                            echo ' class="current"';
                        } ?>><a href="<?php echo $item["url"]; ?>"><span class="icon <?php echo $item['icon']; ?>"></span><span class="btn-text"><?php echo $item["label"]; ?></span></a></li>
                        <?php
                    }
                        ?>
                </ul>
        ...
        <div class="form-content">
            ...
            <form data-content="main-form" class="overflow-auto tabbed-input-column" method="post" data-type="<?php echo get_setting(); ?>" data-action="validate">

                <?php echo G\Render\get_input_auth_token(); ?>
                <?php
                if (is_settings_account()) {
                    if (is_dashboard_user() or is_content_manager()) {
                        ...
                        <ul class="tabbed-content-list table-li">
                            <?php
                                    $banned_ip = CHV\Ip_ban::getSingle(['ip' => get_user()['registration_ip']]);
                        $user_list_values = [
                                        [
                                            'label'        => _s('User ID'),
                        ...
                        if (get_user()['registration_ip']) {
                            $user_list_values[] = [
                                            ...
                                            'content'    => sprintf(str_replace('%IP', '%1$s', '<a href="' . CHV\getSetting('ip_whois_url') . '" target="_blank">%IP</a> · <a href="' . G\get_base_url('search/images/?q=ip:%IP') . '">' . _s('search content') . '</a>  ·  ' . (!$banned_ip ? ('<a data-modal="form" data-args="%IP" data-target="modal-add-ip_ban" data-options=\'{"forced": true}\' data-content="ban_ip">' . _s('Ban IP') . '</a>') : null) . '<span class="' . ($banned_ip ? null : 'soft-hidden') . '" data-content="banned_ip">' . _s('IP already banned') . '</span>'), get_user()['registration_ip'])
                                        ];
                        }
                                    ...
                                    <select name="role" id="role" class="text-input">
                                        <?php
                                                    foreach ([
                                                        ...
                                                        'manager'    => ['label' => _s('Manager'), 'selected' => get_user()['is_manager']],
                    ...
                    <div class="c9 phablet-c1">
                        <div class="input-label">
                            ...
                            <input type="text" name="username" id="username" maxlength="<?php echo CHV\getSetting('username_max_length'); ?>" class="text-input" value="<?php echo get_safe_post()["username"] ?? get_user()["username"]; ?>" pattern="<?php echo CHV\getSetting('username_pattern'); ?>" rel="tooltip" title='<?php _se('%i to %f characters<br>Letters, numbers and "_"', ['%i' => CHV\getSetting('username_min_length'), '%f' => CHV\getSetting('username_max_length')]); ?>' data-tipTip="right" placeholder="<?php _se('Username'); ?>" required>
                        ...
                        <?php
                            if (is_owner() or CHV\Login::isAdmin()) {
                                ?>
                            <div class="input-label">
                                <label for="email"><?php _se('Email address'); ?></label>
                                <input type="email" name="email" id="email" class="text-input" value="<?php echo get_safe_post()["email"] ?? get_user()["email"]; ?>" placeholder="<?php _se('Your email address'); ?>" <?php if (is_email_required()) {
                                    ?> required<?php
                                } ?>>
                                <span class="input-warning red-warning"><?php echo get_input_errors()["email"] ?? ''; ?></span>
                                <?php if (get_changed_email_message()) {
                                    ?><div class="input-below highlight padding-5"><?php echo get_changed_email_message(); ?></div><?php
                                } ?>
                            </div>
                        <?php
                            } ?>
                    ...
                    <?php if (CHV\getSetting('upload_image_exif_user_setting')) {
                        ...
                        <div class="input-label">
                            ...
                            <?php echo CHV\Render\get_checkbox_html([
                                        ...
                                        'checked'    => ((bool) (get_safe_post()['image_keep_exif'] ?? get_user()['image_keep_exif']))
                                    ]); ?>
                        </div>
                    <?php
                            } ?>
                    ...
                    <div class="input-label">
                        ...
                        <?php echo CHV\Render\get_checkbox_html([
                                ...
                                'label'        => _s('Send me emails with news about %s', get_safe_html_website_name()),
                                'checked'    => ((bool) (get_safe_post()['newsletter_subscribe'] ?? get_user()['newsletter_subscribe']))
                            ]); ?>
                    </div>
                    ...
                    <div class="input-label">
                        ...
                        <?php echo CHV\Render\get_checkbox_html([
                                ...
                                'checked'    => ((bool) (get_safe_post()['show_nsfw_listings'] ?? get_user()['show_nsfw_listings']))
                            ]); ?>
                    </div>
                ...
                <?php if (is_settings_powered()) {
                    ...
                    <div class="powered-by powered-by--provider">
                        <div><a href="<?php echo G\get_base_url(); ?>" class="display-inline-block margin-left-auto margin-right-auto"><img src="<?php echo CHV\get_system_image_url($logo_header); ?>" alt=""></a></div>
                        <p><?php _se('This service is provided by %website_name%, we are an independent service provider. Many thanks for your ongoing support.', ['%website_name%' => CHV\getSetting('website_name')]); ?></p>
                    </div>
                    <hr class="line-separator"></hr>
                    <div class="powered-by powered-by--vendor">
                        <div class="display-inline-block margin-left-auto margin-right-auto"><img src="<?php echo G\absolute_to_url(CHV_APP_PATH_CONTENT_SYSTEM . 'chevereto-blue.svg'); ?>" alt="" width="100"></div>
                        <p><?php
                            _se('At %website_name% we are proudly using and supporting %software%.', [
                                '%software%' => CHV\getSetting('enable_powered_by') ? '<a href="https://chevereto.com" target="_blank" rel="nofollow">Chevereto</a>' : 'Chevereto',
                                '%website_name%' => CHV\getSetting('website_name'),
                            ]); ?>
                        </p>
                    </div>
                <?php
                ...
                <?php if (is_settings_password()) {
                    ...
                    <?php
                        // User has password
                        if (isset(get_user()["login"]["password"])) {
                            ?>
                        <div class="c9 phablet-c1">
                            <?php
                                    if (!is_dashboard_user()) {
                            ...
                            <div class="input-label input-password">
                                ...
                                <span class="text-align-right input-warning input-below red-warning<?php echo isset(get_input_errors()["new-password-confirm"]) ? "" : " hidden-visibility"; ?>" data-text="<?php _se("Passwords don't match"); ?>"><?php if (isset(get_input_errors()["new-password-confirm"])) {
                    ...
                    <?php
                        ...
                        <div class="c9 phablet-c1">
                            ...
                            <div class="input-label input-password">
                                ...
                                <span class="input-warning red-warning<?php echo isset(get_input_errors()["new-password-confirm"]) ? "" : " hidden-visibility"; ?>" data-text="<?php _se("Passwords don't match"); ?>"><?php _se("Passwords don't match"); ?></span>
                            </div>
                        </div>
                    <?php
                        } ?>
                ...
                <?php if (is_settings_profile()) {
                    ...
                    <div class="c19 phablet-c1">
                        <div class="input-upload user-settings-avatar">
                            <div class="user-settings-avatar-container grid-columns margin-right-10 phablet-float-left">
                                <?php if (isset(get_user()["avatar"]["url"])) {
                        ?>
                                    <img src="<?php echo get_user()["avatar"]["url"]; ?>" alt="" class="user-image">
                                <?php
                    } else {
                        ?>
                                    <img src="" alt="" class="user-image<?php echo(!isset(get_user()["avatar"]["url"]) ? ' hidden' : ''); ?>">
                                <?php
                    } ?>
                                <span class="user-image default-user-image<?php echo(isset(get_user()["avatar"]["url"]) ? ' hidden' : ''); ?>"><span class="icon fas fa-meh"></span></span>
                                <div class="user-image loading-placeholder hidden"><?php _se('loading'); ?></div>
                            </div>
                            <div class="actions">
                                <div class="btn-container phone-text-align-center">
                                    <a class="btn default" data-trigger="user-avatar-upload"><span class="btn-icon fas fa-camera"></span><span class="btn-text"><?php _se('Upload avatar'); ?></span></a>
                                    <div class="margin-top-5<?php echo(!isset(get_user()["avatar"]) ? ' soft-hidden' : ''); ?>">
                                        <a class="link link--delete" data-action="delete-avatar"><span class="icon fas fa-trash-alt"></span><span class="margin-left-5"><?php _se('Delete'); ?></span></a>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="input-label">
                        ...
                        <?php echo CHV\Render\get_checkbox_html([
                                ...
                                'checked'    => ((bool) (get_safe_post()['is_private'] ?? get_user()['is_private']))
                            ]); ?>
                    </div>
                    ...
                    <div class="c9 phablet-c1">
                        <div class="input-label">
                            ...
                            <input type="text" name="name" id="name" class="text-input" maxlength="60" value="<?php echo get_safe_post()["name"] ?? get_safe_html_user()["name"]; ?>" placeholder="Your real name" required>
                        ...
                        <div class="input-label">
                            ...
                            <input type="url" name="website" id="website" class="text-input" value="<?php
                                echo !empty(get_safe_post())
                                        ? (get_safe_post()["website"] ?? '')
                                        : (get_user()["website_safe_html"] ?? '');
                                
                                ?>" data-validate rel="tooltip" title="<?php _se('http://yourwebsite.com'); ?>" data-tipTip="right" placeholder="<?php _se('http://yourwebsite.com'); ?>">
                            <span class="input-warning red-warning"><?php echo get_input_errors()["website"] ?? ''; ?></span>
                        </div>
                        <div class="input-label">
                            <label for="bio"><?php _se('Bio'); ?></label>
                            <textarea name="bio" id="bio" class="resize-vertical" placeholder="<?php _se('Tell us a little bit about you'); ?>" maxlength="255"><?php echo get_safe_post()["bio"] ?? get_safe_html_user()["bio"]; ?></textarea>
                ...
                if (is_settings_linked_accounts()) {
                    if (is_dashboard_user()) {
                        ?>
                        <p data-content="empty-message" <?php if (count(get_connections()) > 0) {
                            ?>class="soft-hidden" <?php
                        } ?>><?php _se('User has no connections.'); ?></p>
                    <?php
                    } else {
                        ?>
                        <p><?php _se('Link your account to external services to be able to login here.'); ?></p>
                    <?php
                    } ?>
                    <?php
                        ...
                        <div class="account-link account-linked input-label overflow-auto" data-connection="<?php echo $connection['type']; ?>">
                            <h3><span class="btn-icon fab fa-<?php echo $connection['type']; ?>"></span> <?php echo $connection['type_label']; ?> <a class="font-size-small margin-left-5" data-action="disconnect" data-connection="<?php echo $connection['type']; ?>" data-confirm-message="<?php echo $confirm_message; ?>"><?php _se('disconnect'); ?></a></h3>
                    ...
                    <?php
                        if (!is_dashboard_user()) {
                            foreach (get_available_connections() as $connection => $label) {
                                ?>
                            <div class="account-link input-label<?php if (isset(get_connections()[$connection])) {
                                    echo ' soft-hidden';
                                } ?>" data-connect="<?php echo $connection; ?>">
                                <h3><?php echo $label; ?></h3>
                                <a class="link-service link-<?php echo $connection; ?>" href="<?php echo G\get_base_url('connect/' . $connection . '/?return=' . urlencode('settings/linked-accounts')); ?>"><span class="btn-icon fab fa-<?php echo $connection; ?>"></span><span class="sign-text"><?php echo _se('Connect %s', $label); ?></span></a>
                            </div>
                    <?php
                            }
                        } ?>
                ...
                <?php if (!is_settings_linked_accounts() && !is_settings_powered()) {
                    ?>
                    <div class="btn-container btn-container--fixed">
                        <div class="c24 center-box text-align-center">
                            <button class="btn btn-input red" type="submit"><?php _se('Save changes'); ?></button></span>
                        </div>
                    </div>
                <?php
                } ?>
...
<?php if (get_post() and (is_changed() or is_error())) {
    ...
    <script>
        $(function() {
            PF.fn.growl.expirable("<?php echo is_changed() ? (get_changed_message() ? get_changed_message() : _s('Changes have been saved.')) : get_error(); ?>");
        });
    </script>
<?php
                } ?>
