...
<div class="content-width">
	...
	<div class="<?php echo function_exists('is_show_submit') && is_show_submit() ? 'form-content' : ''; ?>">
        ...
        ?>
                ...
                <script type="text/javascript" src="<?php echo CHV\Render\get_static_url(CHV_PATH_PEAFOWL . 'js/apexcharts.js'); ?>"></script>
...
<?php 
    ...
    $dateCumulative = CHV\Stat::getByDateCumulative();
    $imageDataSeries = [];
    $diskDataSeries = [];
    $userDataSeries = [];
    $albumDataSeries = [];
    foreach($dateCumulative as $data) {
        ...
        $diskDataSeries[] = [$timestamp, G\bytes_to_mb($data['disk_used_acc'])];
...
var options = {
    series: [
        {
            name: 'Images',
            data: <?php echo json_encode($imageDataSeries); ?>,
        },
        {
            name: 'Albums',
            data: <?php echo json_encode($albumDataSeries); ?>,
        },
        {
            name: 'Users',
            data: <?php echo json_encode($userDataSeries); ?>,
        },
        {
            name: 'Disk used',
            data: <?php echo json_encode($diskDataSeries); ?>,
        },
    ],
    colors: [
        '#008ffb',
    ...
    theme: {
        mode: '<?php echo G\Handler::getVar('theme_tone'); ?>',
    }
};
...
document.querySelector('#one_year').addEventListener('click', function(e) {
    ...
    chart.zoomX(
        ...
        <?php echo $max; ?>
					...
					<div class="overflow-auto text-align-center margin-top-20 margin-bottom-40">
						<a href="<?php echo G\get_base_url('dashboard/images'); ?>" class="stats-block c6 fluid-column display-inline-block" <?php if (get_totals()['images'] > 999999) {
							...
							<span class="stats-big-number">
								...
								<span class="label"><span class="fas fa-image"></span> <?php _ne('Image', 'Images', get_totals()['images']); ?></span>
							</span>
						</a>
						<a href="<?php echo G\get_base_url('dashboard/albums'); ?>" class="stats-block c6 fluid-column display-inline-block" <?php if (get_totals()['albums'] > 999999) {
							...
							<span class="stats-big-number">
								...
								<span class="label"><span class="fas fa-images"></span> <?php _ne('Album', 'Albums', get_totals()['albums']); ?></span>
							</span>
						</a>
						<a href="<?php echo G\get_base_url('dashboard/users'); ?>" class="stats-block c6 fluid-column display-inline-block" <?php if (get_totals()['users'] > 999999) {
							...
							<span class="stats-big-number">
								...
								<span class="label"><span class="fas fa-users"></span> <?php _ne('User', 'Users', get_totals()['users']); ?></span>
							</span>
						</a>
                    ...
                    <div class="header header-tabs no-select">
                        <h2><i class="fas fa-rss"></i> <?php _se('%s News', 'Chevereto'); ?></h2>
                    </div>
                    ...
                    <div class="header header-tabs no-select">
                        <h2><i class="fas fa-server"></i> <?php _se('Installation details'); ?></h2>
                    </div>
            ...
            case 'bulk':
            ?>
				<div data-content="dashboard-imports" class="margin-top-10 margin-bottom-10">
                <?php
                $statusesDisplay = [
                    'queued' => _s('Queued'),
                    'working' => _s('Working'),
			...
			<div class="fluid-column c3 display-table-cell margin-bottom-0 phone-display-block">
                            ...
                            <div class="pop-box arrow-box arrow-box-top anchor-left">
                                <div class="pop-box-inner pop-box-menu">
                                    <ul>
                                        ...
                                        <li data-content="log-process"><a href="' . G\get_base_url('importer-jobs/%id%/process') . '" target="_blank">' . _s('Process log') . '</a></li>
                                        <li data-content="log-errors"><a href="' . G\get_base_url('importer-jobs/%id%/errors') . '" target="_blank"><span class="icon fas fa-exclamation-triangle color-red"></span> ' . _s('Errors') . '</a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                ...
                <div class ="text-content">
                    ...
                    <?php
                        $code = getenv('CHEVERETO_SERVICING') == 'docker'
                            ? "docker exec -it --user www-data \\\n\tchv-container \\\n\tphp cli.php -C importing"
                            : 'sudo -u www-data php '.G_ROOT_PATH.'cli.php -C importing'
                    ?>
                    <div class="margin-bottom-10">
                        <code class="code display-block" style="white-space: pre-wrap;"><?php echo $code; ?></code>
                    </div>
                    <p><?php _se('Read the %s for more information about this feature.', get_docs_link('features/content/bulk-content-importer.html', _s('documentation'))); ?></p>
                </div>
            </div>
				<script>
					...
					<?php if (get_dashboard() == 'users') {
						...
						<div class="header-content-right phone-float-none">
							<div class="list-selection header--centering">
								<a class="header-link" data-modal="form" data-target="modal-add-user"><span class="fas fa-user-plus"></span> <?php _se('Add user'); ?></a>
							</div>
						</div>
            ...
            case 'settings':
                function personal_mode_warning()
                {
                    if (CHV\getSetting('website_mode') === 'personal') {
                        echo '<div class="input-below"><span class="icon fas fa-info-circle color-red"></span> ' . _s('This setting is always diabled when using personal website mode.') . '</div>';
                    }
                }
            ?>
				<form id="dashboard-settings" method="post" data-type="<?php echo get_dashboard(); ?>" data-action="validate" enctype="multipart/form-data">
					...
					<div class="header header--centering default-margin-bottom follow-scroll no-select">
						<h1>
							<span class="icon fas fa-cogs phablet-hide tablet-hide laptop-hide desktop-hide"></span>
						...
						<div data-content="pop-selection" class="pop-btn pop-keep-click header-link float-left margin-left-5" data-action="settings-switch">
                            ...
                            <div class="pop-btn-text margin-left-5">
                                <span class="icon <?php echo get_settings()['icon']; ?>"></span>
                                <span class="btn-text"><?php echo get_settings()['label']; ?></span>
								...
								<div class="pop-box-inner pop-box-menu pop-box-menucols">
									<ul>
                                        ...
                                        foreach (get_settings_menu() as $item) {
                                            ?>
											<li class="with-icon<?php if ($item['current']) {
                                                echo ' current';
                                            } ?>"><a href="<?php echo $item['url']; ?>"><span class="btn-icon <?php echo $item['icon']; ?>"></span><span class="btn-text"><?php echo $item['label']; ?></span></a></li>
											<?php
                                        }
                                            ?>
									</ul>
                    ...
                    if (get_dashboard() == 'settings') {
                        ...
                        if (get_settings()['key'] == 'pages') {
                                        ...
                                        case 'edit':
                                            ...
                                            if(in_array($key, ['page_file_path_absolute','page_file_path'])) {
                                                if(G\get_app_setting('disable_php_pages')) {
                                                    $return = G\str_replace_last('.php', '.html', $return);
                                                }
                                            }
                                            return $return;
                                            break;
							...
							<?php if (isset(get_settings_pages()['title'])) {
                                ?>
							...
							<?php if (get_settings_pages()['doing'] !== 'listing') {
                                ...
                                $page_internal_combo_visible = get_settings_pages()['doing'] == 'edit' ? (get_page_val('page_type') == 'internal') : true; ?>
                                    ...
                                    echo ' soft-hidden';
										...
										<div class="input-label">
											...
											<div class="c9 phablet-c1"><input type="text" name="page_file_path" id="page_file_path" class="text-input" value="<?php echo get_page_val('page_file_path'); ?>" pattern="^[\w]([\w-]*[\w])?(\/[\w]([\w-]*[\w])?)*\.<?php echo G\get_app_setting('disable_php_pages') ? 'html' : '(php|html)'; ?>$" <?php echo $page_internal_combo_visible ? 'required' : 'data-required'; ?> placeholder="page.<?php echo G\get_app_setting('disable_php_pages') ? 'html' : '(php|html)'; ?>"></div>
								...
								<div class="input-label">
									...
									<div class="input-below"><?php _se('Check the <a %s>icon reference</a> for the complete list of supported icons.', 'href="https://fontawesome.com/icons?d=gallery&p=2&m=free" target="_blank"'); ?></div>
								</div>
									...
									<ul data-content="dashboard-categories-list" class="tabbed-content-list table-li-hover table-li margin-top-20 margin-bottom-20">
                                        ...
                                        foreach (get_pages() as $k => $v) {
											...
											<li>
												...
												<span class="c13 display-table-cell padding-right-10"><a href="<?php echo G\get_base_url('dashboard/settings/pages/edit/' . $v['id']); ?>"><?php echo G\truncate($v['title'], 64); ?></a></span>
						...
						<?php if (get_settings()['key'] == 'image-upload') { ?>
									...
									<ul class="c20 phablet-c1">
                                        ...
                                        foreach (CHV\Upload::getAvailableImageFormats() as $k) {
                                            ...
                                            echo strtr('<li class="c5 display-inline-block margin-right-10"><label class="display-block" for="image_format_enable[%k]" %tip> <input type="checkbox" name="image_format_enable[]" id="image_format_enable[%k]" value="%k" %checked %disabled>%K</label></li>', [
                                                '%k' => $k,
                                                '%K' => strtoupper($k),
                                                '%checked' => (in_array($k, CHV\Upload::getEnabledImageFormats()) ? 'checked' : ''),
                            ...
                            <div class="input-label">
								...
								<div class="c5 phablet-c1">
                                    <select type="text" name="enable_uploads_url" id="enable_uploads_url" class="text-input">
										<?php
                                        echo CHV\Render\get_select_options_html([1 => _s('Enabled'), 0 => _s('Disabled')], CHV\Settings::get('enable_uploads_url')); ?>
									</select></div>
							...
							<div class="input-label">
								...
								<div class="c5 overflow-auto clear-both">
									<div class="c2 float-left">
										<input type="number" min="0" pattern="\d+" name="upload_max_image_width" id="upload_max_image_width" class="text-input" value="<?php echo get_safe_post()['upload_max_image_width'] ?? CHV\Settings::get('upload_max_image_width'); ?>" placeholder="<?php echo  CHV\Settings::getDefault('upload_max_image_width'); ?>" rel="tooltip" data-tiptip="top" title="<?php _se('Width'); ?>" required>
									</div>
									<div class="c2 float-left margin-left-10">
										<input type="number" min="0" pattern="\d+" name="upload_max_image_height" id="upload_max_image_height" class="text-input" value="<?php echo get_safe_post()['upload_max_image_height'] ?? CHV\Settings::get('upload_max_image_height'); ?>" placeholder="<?php echo  CHV\Settings::getDefault('upload_max_image_height'); ?>" rel="tooltip" data-tiptip="top" title="<?php _se('Height'); ?>" required>
									</div>
								</div>
							...
							<div class="input-label">
								...
								<div class="c2"><input type="number" min="0.1" step="0.1" max="<?php echo G\bytes_to_mb(CHV\Settings::get('true_upload_max_filesize')); ?>" pattern="\d+" name="upload_max_filesize_mb" id="upload_max_filesize_mb" class="text-input" value="<?php echo get_safe_post()['upload_max_filesize_mb'] ?? CHV\Settings::get('upload_max_filesize_mb'); ?>" placeholder="MB" required></div>
							...
							<div class="input-label">
								...
								<div class="c9 phablet-c1"><input type="text" name="upload_image_path" id="upload_image_path" class="text-input" value="<?php echo get_safe_post()['upload_image_path'] ?? CHV\Settings::get('upload_image_path'); ?>" placeholder="<?php _se('Relative to Chevereto root'); ?>" required></div>
							...
							<div class="input-label">
								...
								<div class="c5 overflow-auto clear-both">
									<div class="c2 float-left">
										<input type="number" min="16" pattern="\d+" name="upload_thumb_width" id="upload_thumb_width" class="text-input" value="<?php echo get_safe_post()['upload_thumb_width'] ?? CHV\Settings::get('upload_thumb_width'); ?>" placeholder="<?php echo  CHV\Settings::getDefault('upload_thumb_width'); ?>" rel="tooltip" data-tiptip="top" title="<?php _se('Width'); ?>" required>
									</div>
									<div class="c2 float-left margin-left-10">
										<input type="number" min="16" pattern="\d+" name="upload_thumb_height" id="upload_thumb_height" class="text-input" value="<?php echo get_safe_post()['upload_thumb_height'] ?? CHV\Settings::get('upload_thumb_height'); ?>" placeholder="<?php echo  CHV\Settings::getDefault('upload_thumb_height'); ?>" rel="tooltip" data-tiptip="top" title="<?php _se('Height'); ?>" required>
									</div>
								</div>
							...
							<div class="input-label">
								...
								<div class="c2">
									<input type="number" min="16" pattern="\d+" name="upload_medium_size" id="upload_medium_size" class="text-input" value="<?php echo get_safe_post()['upload_medium_size'] ?? CHV\Settings::get('upload_medium_size'); ?>" placeholder="<?php echo CHV\Settings::getDefault('upload_medium_size'); ?>" required>
								</div>
								...
								<div data-combo-value="1" class="switch-combo phablet-c1<?php if ((get_safe_post() ? get_safe_post()['watermark_enable'] : CHV\Settings::get('watermark_enable')) != 1) {
									...
									<div class="input-label">
										...
										<div class="c5 phablet-c1"><select type="text" name="watermark_position" id="watermark_position" class="text-input">
                                                ...
                                                ], get_safe_post()['watermark_position'] ?? CHV\Settings::get('watermark_position')); ?>
											</select></div>
									...
									<div class="input-label">
										...
										<div class="c2">
											<input type="number" min="1" max="100" pattern="\d+" name="watermark_percentage" id="watermark_percentage" class="text-input" value="<?php echo get_safe_post()['watermark_percentage'] ?? CHV\Settings::get('watermark_percentage'); ?>" placeholder="<?php echo CHV\Settings::getDefault('watermark_percentage'); ?>" required>
										</div>
									...
									<div class="input-label">
										...
										<div class="c2">
											<input type="number" min="0" pattern="\d+" name="watermark_margin" id="watermark_margin" class="text-input" value="<?php echo get_safe_post()['watermark_margin'] ?? CHV\Settings::get('watermark_margin'); ?>" placeholder="<?php echo CHV\Settings::getDefault('watermark_margin'); ?>" required>
										</div>
									...
									<div class="input-label">
										...
										<div class="c2">
											<input type="number" min="1" max="100" pattern="\d+" name="watermark_opacity" id="watermark_opacity" class="text-input" value="<?php echo get_safe_post()['watermark_opacity'] ?? CHV\Settings::get('watermark_opacity'); ?>" placeholder="<?php echo CHV\Settings::getDefault('watermark_opacity'); ?>" required>
										</div>
					...
					<span class="c2 display-table-cell"><a class="delete-link" data-ip_ban-id="%ID%" data-args="%ID%" data-confirm="' . _s("Do you really want to remove the ban to the IP %s? This can't be undone.") . '" data-submit-fn="CHV.fn.ip_ban.delete.submit" data-before-fn="CHV.fn.ip_ban.delete.before" data-ajax-deferred="CHV.fn.ip_ban.delete.complete">' . _s('Delete') . '</a></span>
						...
						<?php if (get_settings()['key'] == 'users') { ?>
							...
							<div class="input-label">
								...
								<div class="c3"><input type="number" min="0" pattern="\d+" name="user_minimum_age" id="user_minimum_age" class="text-input" <?php if (CHV\getSetting('website_mode') == 'personal') {
                                            echo ' disabled';
                                        } ?> value="<?php echo get_safe_post()['user_minimum_age'] ?? CHV\Settings::get('user_minimum_age'); ?>" placeholder="<?php _se('Empty'); ?>"></div>
							...
							<div class="input-label">
								...
								<div class="c3"><input type="number" min="0" pattern="\d+" name="user_image_avatar_max_filesize_mb" id="user_image_avatar_max_filesize_mb" class="text-input" value="<?php echo get_safe_post()['user_image_avatar_max_filesize_mb'] ?? CHV\Settings::get('user_image_avatar_max_filesize_mb'); ?>" placeholder="MB" required></div>
							...
							<div class="input-label">
								...
								<div class="c3"><input type="number" min="0" pattern="\d+" name="user_image_background_max_filesize_mb" id="user_image_background_max_filesize_mb" class="text-input" value="<?php echo get_safe_post()['user_image_background_max_filesize_mb'] ?? CHV\Settings::get('user_image_background_max_filesize_mb'); ?>" placeholder="MB" required></div>
							...
							<div id="flood-protection-combo">
								<div data-combo-value="1" class="switch-combo <?php if (!(get_safe_post() ? get_safe_post()['flood_uploads_protection'] : CHV\Settings::get('flood_uploads_protection'))) {
									...
									<div class="input-label">
										...
										<div class="c3"><input type="number" min="0" name="flood_uploads_minute" id="flood_uploads_minute" class="text-input" value="<?php echo get_safe_post()['flood_uploads_minute'] ?? CHV\Settings::get('flood_uploads_minute', true); ?>" placeholder="<?php echo CHV\Settings::getDefault('flood_uploads_minute'); ?>"></div>
									...
									<div class="input-label">
										...
										<div class="c3"><input type="number" min="0" name="flood_uploads_hour" id="flood_uploads_hour" class="text-input" value="<?php echo get_safe_post()['flood_uploads_hour'] ?? CHV\Settings::get('flood_uploads_hour', true); ?>" placeholder="<?php echo CHV\Settings::getDefault('flood_uploads_hour'); ?>"></div>
									...
									<div class="input-label">
										...
										<div class="c3"><input type="number" min="0" name="flood_uploads_day" id="flood_uploads_day" class="text-input" value="<?php echo get_safe_post()['flood_uploads_day'] ?? CHV\Settings::get('flood_uploads_day', true); ?>" placeholder="<?php echo CHV\Settings::getDefault('flood_uploads_day'); ?>"></div>
									...
									<div class="input-label">
										...
										<div class="c3"><input type="number" min="0" name="flood_uploads_week" id="flood_uploads_week" class="text-input" value="<?php echo get_safe_post()['flood_uploads_week'] ?? CHV\Settings::get('flood_uploads_week', true); ?>" placeholder="<?php echo CHV\Settings::getDefault('flood_uploads_week'); ?>"></div>
									...
									<div class="input-label">
										...
										<div class="c3"><input type="number" min="0" name="flood_uploads_month" id="flood_uploads_month" class="text-input" value="<?php echo get_safe_post()['flood_uploads_month'] ?? CHV\Settings::get('flood_uploads_month', true); ?>" placeholder="<?php echo CHV\Settings::getDefault('flood_uploads_month'); ?>"></div>
						...
						<?php if (get_settings()['key'] == 'theme') { ?>
							...
							<div class="input-label">
								...
								<div class="c2"><input type="number" min="0" pattern="\d+" name="image_load_max_filesize_mb" id="image_load_max_filesize_mb" class="text-input" value="<?php echo get_safe_post()['image_load_max_filesize_mb'] ?? CHV\Settings::get('image_load_max_filesize_mb'); ?>" placeholder="MB"></div>
						...
						<?php if (get_settings()['key'] == 'banners') {
                            ?>
							<p><?php echo read_the_docs_settings('banners', _s('Banners')); ?></p>
							<?php
                            $banners = get_banners();
            ...
            <p><?php echo read_the_docs_settings('external-storage', _s('External storage')); ?></p>
			...
			<div id="mail-combo">
				...
				<div data-combo-value="smtp" class="switch-combo c9 phablet-c1<?php if ((get_safe_post() ? get_safe_post()['email_mode'] : CHV\Settings::get('email_mode')) !== 'smtp') {
					...
					<div class="input-label">
						...
						<div class="overflow-auto">
							<div class="c7 float-left">
								<input type="text" name="email_smtp_server" id="email_smtp_server" class="text-input" value="<?php echo get_safe_post()['email_smtp_server'] ?? CHV\Settings::get('email_smtp_server'); ?>" placeholder="<?php _se('SMTP server'); ?>">
							</div>
							<div class="c2 float-left margin-left-10">
					...
					<div class="input-label">
						...
						<input type="text" name="email_smtp_server_username" id="email_smtp_server_username" class="text-input" value="<?php echo get_safe_post()['email_smtp_server_username'] ?? CHV\Settings::get('email_smtp_server_username'); ?>">
					...
					<div class="input-label">
						...
						<input type="password" name="email_smtp_server_password" id="email_smtp_server_password" class="text-input" value="<?php echo get_safe_post()['email_smtp_server_password'] ?? CHV\Settings::get('email_smtp_server_password'); ?>">
      ...
      <?php
			...
			<?php if (get_settings()['key'] == 'social-networks') {
                            ?>
			<p><?php echo read_the_docs_settings('social-networks', _s('Social networks')); ?></p>
			<div class="input-label">
			...
			<div id="facebook-combo">
				<div data-combo-value="1" class="switch-combo c9 phablet-c1<?php if (!(get_safe_post() ? get_safe_post()['facebook'] : CHV\Settings::get('facebook'))) {
					...
					<div class="input-label">
						...
						<input type="text" name="facebook_app_id" id="facebook_app_id" class="text-input" value="<?php echo get_safe_post()['facebook_app_id'] ?? CHV\Settings::get('facebook_app_id', true); ?>">
					...
					<div class="input-label">
						...
						<input type="text" name="facebook_app_secret" id="facebook_app_secret" class="text-input" value="<?php echo get_safe_post()['facebook_app_secret'] ?? CHV\Settings::get('facebook_app_secret', true); ?>">
			...
			<div id="twitter-combo">
				<div data-combo-value="1" class="switch-combo c9 phablet-c1<?php if (!(get_safe_post() ? get_safe_post()['twitter'] : CHV\Settings::get('twitter'))) {
					...
					<div class="input-label">
						...
						<input type="text" name="twitter_api_key" id="twitter_api_key" class="text-input" value="<?php echo get_safe_post()['twitter_api_key'] ?? CHV\Settings::get('twitter_api_key', true); ?>">
					...
					<div class="input-label">
						...
						<input type="text" name="twitter_api_secret" id="twitter_api_secret" class="text-input" value="<?php echo get_safe_post()['twitter_api_secret'] ?? CHV\Settings::get('twitter_api_secret', true); ?>">
			...
			<div class="input-label">
				...
				<div class="c5 phablet-c1">
					<input type="text" name="twitter_account" id="twitter_account" class="text-input" placeholder="chevereto" value="<?php echo get_safe_post()['twitter_account'] ?? CHV\Settings::get('twitter_account', true); ?>">
				</div>
			...
			<div id="google-combo">
				<div data-combo-value="1" class="switch-combo c9 phablet-c1<?php if (!(get_safe_post() ? get_safe_post()['google'] : CHV\Settings::get('google'))) {
					...
					<div class="input-label">
						...
						<input type="text" name="google_client_id" id="google_client_id" class="text-input" value="<?php echo get_safe_post()['google_client_id'] ?? CHV\Settings::get('google_client_id', true); ?>">
					...
					<div class="input-label">
						...
						<input type="text" name="google_client_secret" id="google_client_secret" class="text-input" value="<?php echo get_safe_post()['google_client_secret'] ?? CHV\Settings::get('google_client_secret', true); ?>">
			...
			<div id="vk-combo">
				<div data-combo-value="1" class="switch-combo c9 phablet-c1<?php if (!(get_safe_post() ? get_safe_post()['vk'] : CHV\Settings::get('vk'))) {
					...
					<div class="input-label">
						...
						<input type="text" name="vk_client_id" id="vk_client_id" class="text-input" value="<?php echo get_safe_post()['vk_client_id'] ?? CHV\Settings::get('vk_client_id', true); ?>">
					...
					<div class="input-label">
						...
						<input type="text" name="vk_client_secret" id="vk_client_secret" class="text-input" value="<?php echo get_safe_post()['vk_client_secret'] ?? CHV\Settings::get('vk_client_secret', true); ?>">
			...
			<div id="akismet-combo" class="c9 phablet-c1">
				<div data-combo-value="1" class="switch-combo<?php if (!(get_safe_post() ? get_safe_post()['akismet'] : CHV\Settings::get('akismet'))) {
					...
					<div class="input-label">
						...
						<input type="text" name="akismet_api_key" id="akismet_api_key" class="text-input" value="<?php echo get_safe_post()['akismet_api_key'] ?? CHV\Settings::get('akismet_api_key', true); ?>" >
			...
			<div id="cdn-combo" class="c9 phablet-c1">
				<div data-combo-value="1" class="switch-combo<?php if (!(get_safe_post() ? get_safe_post()['cdn'] : CHV\Settings::get('cdn'))) {
					...
					<div class="input-label">
						...
						<input type="text" name="cdn_url" id="cdn_url" class="text-input" value="<?php echo get_safe_post()['cdn_url'] ?? CHV\Settings::get('cdn_url', true); ?>" placeholder="http://something.netdna-cdn.com/">
				...
				<div data-combo-value="1" class="switch-combo<?php if (!(get_safe_post() ? get_safe_post()['recaptcha'] : CHV\Settings::get('recaptcha'))) {
					...
					<div class="c9 phablet-c1">
						<div class="input-label">
							...
							<input type="text" name="recaptcha_public_key" id="recaptcha_public_key" class="text-input" value="<?php echo get_safe_post()['recaptcha_public_key'] ?? CHV\Settings::get('recaptcha_public_key', true); ?>">
						...
						<div class="input-label">
							...
							<input type="text" name="recaptcha_private_key" id="recaptcha_private_key" class="text-input" value="<?php echo get_safe_post()['recaptcha_private_key'] ?? CHV\Settings::get('recaptcha_private_key', true); ?>">
					...
					<div class="input-label">
						<div class="c9 phablet-c1">
							...
							<div class="c2">
								<input type="number" min="0" name="recaptcha_threshold" id="recaptcha_threshold" class="text-input" value="<?php echo get_safe_post()['recaptcha_threshold'] ?? CHV\Settings::get('recaptcha_threshold'); ?>">
							</div>
						</div>
				...
				<div data-combo-value="disqus" class="switch-combo<?php if ((get_safe_post() ? get_safe_post()['comments_api'] : CHV\Settings::get('comments_api')) !== 'disqus') {
					...
					<div class="c9 phablet-c1">
						<div class="input-label">
							...
							<input type="text" name="disqus_shortname" id="disqus_shortname" class="text-input" value="<?php echo get_safe_post()['disqus_shortname'] ?? CHV\Settings::get('disqus_shortname', true); ?>">
						...
						<div class="input-label">
							...
							<input type="text" name="disqus_secret_key" id="disqus_secret_key" class="text-input" value="<?php echo get_safe_post()['disqus_secret_key'] ?? CHV\Settings::get('disqus_secret_key', true); ?>">
						...
						<div class="input-label">
							...
							<input type="text" name="disqus_public_key" id="disqus_public_key" class="text-input" value="<?php echo get_safe_post()['disqus_public_key'] ?? CHV\Settings::get('disqus_public_key', true); ?>">
            ...
            <div id="moderatecontent-combo" class="c12 phablet-c1">
                <div data-combo-value="1" class="switch-combo<?php if ((get_safe_post() ? get_safe_post()['moderatecontent'] : CHV\Settings::get('moderatecontent')) == 0) {
                    ...
                    <div class="input-label">
                        ...
                        <input type="text" name="moderatecontent_key" id="moderatecontent_key" class="text-input" value="<?php echo get_safe_post()['moderatecontent_key'] ?? CHV\Settings::get('moderatecontent_key'); ?>" placeholder="">
