...
<?php
    if (isset(get_user()["background"]) or is_owner() or is_content_manager()) {
        ?>
<div id="background-cover" data-content="user-background-cover"<?php if (!isset(get_user()["background"])) {
            ?> class="no-background"<?php
        } ?>>
	<div id="background-cover-wrap">
		<div id="background-cover-src" data-content="user-background-cover-src"<?php if (isset(get_user()["background"], get_user()["background"]["url"])) {
            ?> style="background-image: url('<?php echo get_user()["background"]["url"]; ?>');"<?php
        } ?>></div>
        <?php
            if (is_owner() or is_content_manager()) {
                ?>
        <div class="content-width">
            <div data-content="user-upload-background" class="btn btn-input default<?php if (isset(get_user()["background"])) {
                        ?> hidden<?php
            ...
            <div id="change-background-cover" data-content="user-change-background" class="pop-btn<?php if (!isset(get_user()["background"])) {
                        ?> hidden<?php
                    } ?>">
...
<div class="content-width">
	...
	<?php CHV\Render\show_banner('user_after_top', get_list()->sfw ?? true); ?>

	<div id="top-user" class="top-user<?php echo !isset(get_user()["background"]) ? ' no-background' : ''; ?>">
		<div class="top-user-credentials">
			...
			<div class="user-meta"><span class="user-social-networks"><?php if (isset(get_user()["twitter"])) {
                    ?><a class="fab fa-twitter" href="<?php echo get_user()["twitter"]["url"]; ?>" rel="nofollow" target="_blank"></a><?php
                } if (isset(get_user()["facebook"])) {
                    ?><a class="icon fab fa-facebook" href="<?php echo get_user()["facebook"]["url"]; ?>" rel="nofollow" target="_blank"></a><?php
                } if (isset(get_user()["website"])) {
                    ?><a class="icon fas fa-globe-americas" href="<?php echo get_user()['website_display']; ?>"<?php echo !get_user()['is_admin'] ? ' rel="nofollow"' : null; ?> target="_blank"></a><?php
                } ?></span><?php
                if (is_owner() or is_admin() or (is_content_manager() && get_user()['is_content_manager'] == false)) {
                    ?>
				<a class="link link--edit" href="<?php echo G\get_base_url(is_owner() ? 'settings/profile' : 'dashboard/user/' . get_user()['id']); ?>"><span class="icon fas fa-edit"></span><span class="margin-left-5"><?php _se('Edit'); ?></span></a>
			<?php
                    if (!is_owner() and is_content_manager()) {
                        ?>
				<a class="link link--delete" data-confirm="<?php _se("Do you really want to delete this user? This can't be undone."); ?>" data-submit-fn="CHV.fn.submit_resource_delete" data-ajax-deferred="CHV.fn.complete_resource_delete" data-ajax-url="<?php echo G\get_base_url("json"); ?>"><span class="icon fas fa-trash-alt"></span><span class="margin-left-5"><?php _se('Delete'); ?></span></a>
			<?php
                    } ?>
			<?php
                }
			...
			<div class="user-meta margin-bottom-5">
				...
				<a class="btn-follow" data-followed="<?php echo (int)get_user()['followed']; ?>" data-action="follow">
					...
					<span class="btn btn-capsule btn-unfollowed blue outline"><span class="btn-icon fas fa-user-plus"></span><span class="btn-text"><?php _se('Follow'); ?></span></span>
				</a>
				<?php
                    } ?>
			</div>
		...
		<div class="header-content-right phone-float-none">
			<div class="text-align-right">
                ...
                ?>
				<a class="number-figures" href="<?php echo get_user()["url"]; ?>/liked"><span class="icon fas fa-heart"></span> <b data-text="likes-count"><?php echo get_user()["liked"]; ?></b></a>
				<?php
            } ?>
			</div>
			...
			<?php
                if (is_owner()) {
                    ?>
			<div class="text-align-right">
				<button class="btn default" data-modal="edit" data-target="new-album"><span class="btn-icon fas fa-images"></span><span class="btn-text"><?php _se('Create new album'); ?></span></button>
	...
	<?php CHV\Render\show_banner('user_before_listing', get_list()->sfw ?? true); ?>

    <div class="header">
        <h1>
            <strong><?php echo  get_title(); ?></strong>
        </h1>
    </div>
	...
	<div class="header header-tabs follow-scroll">
		...
		<?php
                ...
                ?>
		<a href="<?php echo get_user()["url"]; ?>" class="user-image margin-right-5 float-left">
			<?php if (isset(get_user()["avatar"])) {
                    ?>
    ...
    <?php G\Render\include_theme_file("snippets/tabs"); ?>
		...
		<?php
            if (is_show_user_items_editor() or (is_owner() or is_content_manager())) {
...
<?php if ((is_owner() or is_content_manager()) and isset($_REQUEST["deleted"])) {
              ?>
<script>
$(function() {
