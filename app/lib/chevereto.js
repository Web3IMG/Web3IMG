...
$(function () {
    ...
    var ThemeTone = {
        ...
        toggle: function () {
            ...
            var fromTone = this.isDark() ? "dark" : "light";
            $("html")
                .removeClass(this.tones[fromTone].html)
                .addClass(this.tones[toTone].html);
            $(".apexcharts-theme-" + fromTone)
                .removeClass("apexcharts-theme-" + fromTone)
                .addClass("apexcharts-theme-" + toTone)
    ...
    $(document).on("click", "[data-action=list-select-all]", function () {
        CHV.fn.list_editor.selectItem($(PF.obj.listing.selectors.list_item + ":visible:not(.selected)"));
            ...
            if (
                ...
                PF.fn.modal.simple({
                    ...
                    message:
                        ...
                        '        <label for="system-update">' +
                        '            <input type="hidden" name="system-update" value="0" checked>' +
                        '            <input type="checkbox" name="system-update" id="system-update" value="1">' + PF.fn._s('I confirm that this server meets the %s for this update', '<a href="https://chv.to/v3reqs" target="_blank">' + PF.fn._s('system requirements') + '</a>') +
                        '        </label>' +
                        '    </div>' +
                        '</div>' +
                        '<div class="btn-container margin-bottom-0"><a data-action="system-update" href="' +
                        PF.obj.config.base_url +
                        "/update/" +
                        "?auth_token=" +
                        PF.obj.config.auth_token +
                        '" class="btn btn-input red">' +
                        PF.fn._s("Update now") +
                        '</a></div>',
                    html: true,
                });
                ...
                PF.fn.growl.call(
                    PF.fn._s(
                        "This website is running latest %s version" + " (V3.X)",
                        CHEVERETO.edition
                    )
                );
            }
    ...
    if (
        ...
        $.getScript(
            "https://www.recaptcha.net/recaptcha/api.js?onload=reCaptchaCallback&render=explicit"
        );
    }
    ...
    $(document).on("contextmenu", ".list-item .image-container", function(e) {
        ...
        CHV.fn.list_editor.blink($list_item);
        CHV.fn.list_editor.toggleSelectItem($list_item);
        e.preventDefault();
    });
});
...
CHV.fn.modal = {
    getTemplateWithPreview: function(selector, $target) {
        ...
        if(typeof src !== typeof undefined) {
            html += '<a href="' + $target.attr('data-url-short') + '" target="_blank"><img class="canvas" src='+ src +' /></a>';
        }
    ...
    getTemplateWithPreviews: function(selector, $targets, limit = 50) {
        ...
        $targets.each(function () {
            ...
            html += '<a class="image-preview-container" href="' + $(this).attr('data-url-short') + '" target="_blank">';
            ...
            if(src.exists()) {
                ...
                html += '<canvas width="160" height="160" class="thumb ee" />';
...
CHV.fn.listingViewer = {
    ...
    filler: function (isOpened) {
        ...
        $.each($tools.find("[data-action]"), function (i, v) {
            ...
            if(typeof keymap == typeof undefined) {
                return false;
            }
            $(
...
CHV.fn.bindSelectableItems = function () {
    ...
    $("html.device-nonmobile " + sel).selectable({
        ...
        cancel:
            ".content-empty, .header, #tab-share, #tab-info, .viewer-title, .header-link, .top-bar, .content-listing-pagination *, #fullscreen-modal, #top-user, #background-cover, .list-item-desc, .list-item-image-tools, [data-action=load-image], #tab-embeds",
...
CHV.fn.list_editor = {
    blink: function($target) {
        // var $target = $target.find('.image-container');
