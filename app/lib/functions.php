...
// Move to render?
function getSetting($key = '', $safe = false)
{
...
function getSystemNotices()
{
    ...
    if (!in_array($_SERVER['SERVER_ADDR'], ['127.0.0.1', '::1'])) {
        ...
        if (preg_match('/@chevereto\.example/', getSetting('email_from_email')) || preg_match('/@chevereto\.example/', getSetting('email_incoming_email'))) {
...
{
    $dirname = dirname($sub);
    $local = CHV_ROOT_URL_STATIC;
    $url = AssetStorage::getStorage()['url'] ?? $local;
    if(basename($dirname) == 'default') {
        $url = $local;
    }
    return G\absolute_to_url(CHV_PATH_CONTENT . $sub, $url);
}
...
{
    $remove_old = true;
    $localPath = CHV_PATH_CONTENT_IMAGES_SYSTEM;
    ...
    if(!isset($typeArr[$what]) && !G\starts_with('homepage_cover_image_', $what)) {
        throw new OutOfBoundsException(sprintf('Invalid key %s', $what));
    }
    ...
    foreach ($typeArr as $k => &$v) {
        $v['name'] .= '_' . number_format(round(microtime(true) * 1000), 0, '', '') . '_' . G\random_string(6);
    }
    ...
    Storage::uploadFiles($toStorage, AssetStorage::getStorage(), [
    ...
    if ($remove_old) {
        if($what === 'watermark_image') {
            unlinkIfExists(CHV_PATH_CONTENT_IMAGES_SYSTEM . $db_filename);
        } else {
            Storage::deleteFiles($storagePath . $db_filename, AssetStorage::getStorage());
        }
    }
...
function isSafeToExecute($max_execution_time = null, $options = [])
{
    ...
    $executed_time = microtime(true) - G_APP_TIME_EXECUTION_START;
...
function getJsModLangL10n()
{
    foreach (new DirectoryIterator(CHV_APP_PATH_CONTENT_LANGUAGES . 'cache/') as $fileInfo) {
        ...
        $lang_code = str_replace('.po.cache.php', '', $fileInfo->getFilename());
...
function get_base_url_wildcard($path = '', $subdomain = null)
{
    if ($subdomain == null && defined('CHV_HOST_WILDCARD')) {
        $subdomain = CHV_HOST_WILDCARD;
    }
    if ($subdomain) {
        return G\str_replace_first(CHV_HTTP_HOST, $subdomain . '.' . CHV_HTTP_HOST, G\get_base_url($path));
    }

    return G\get_base_url($path);
}

function get_current_url_wildcard($subdomain, $qsremove = [])
{
    return G\str_replace_first(CHV_HTTP_HOST, $subdomain . '.' . CHV_HTTP_HOST, G\get_current_url(true, $qsremove));
}
/**
