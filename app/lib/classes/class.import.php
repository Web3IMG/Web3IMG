                    ...
                    try {
                        ...
                        switch ($this->parse) {
                            case 'user':
                                ...
                                if (isset($user) && isset($this->metadata['profileImages'])) {
                                    try {
                            ...
                            case 'album':
                                ...
                                if($insertId == 0) {
                                    ...
                                    $this->logProcess(
                                        $parent_lock === null
                                            ? 'No parent lock'
                                            : 'Parent ' . $parent_lock['content_type'] . ' id ' . $parent_lock['content_id']
                                    );
                                    if(isset($parent_lock)) {
                                        if($parent_lock['content_type'] === 'user') {
                                            $user_id = $parent_lock['content_id'];
                                        } else {
                                            $user_id = DB::get('albums', ['id' => $parent_lock['content_id']])[0]['album_user_id'] ?? null;
                                            $this->logProcess("User id $user_id taken from parent context");
            ...
            if ($this->parse == 'image') {
                ...
                try {
                    ...
                    if (!file_exists($metaFile)) {
                        $metaFile = G\change_pathname_extension($pathName, 'json');
                    }
                    ...
                    if ($e->getCode() == 666) {
                        ...
                        $this->logException($e);
                        $failedPathName = G\str_replace_first($this->path, $failedPath, $pathName);
    ...
    public function parseMetadata($filename, $type = null)
    {
        ...
        if ($metadata = $metadata[$metadataKey]) {
            ...
            foreach ($tr as $metaProp => $metaValue) {
                ...
                $propValue = $metadata[is_array($val) ? ($val[0] ?? null) : $val] ?? null;
                if (isset($propValue)) {
