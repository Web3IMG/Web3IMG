...
$(function () {
    ...
    window.addEventListener("scroll", function () {
        ...
        setTimeout(function () {
            ...
            $("#top-bar, .follow-scroll").each(function() {
                if($(this).attr("data-scroll-lock") === "1") {
                    return;
                }
                $(this)
                    .addClass(down ? "scroll-down" : "scroll-up")
                    .removeClass(down ? "scroll-up" : "scroll-down");
            });            
            scrollTimer = false;
        }, 250);
    });
                ...
                if (!$(".pop-box-header", $pop_box).exists()) {
                    $pop_box.prepend(
                        $("<div/>", {
                            ...
                            html: text + '<span class="btn-icon icon--close fas fa-times"></span></span>'
                        })
                    );
                }
            } else {
    ...
    $(document).on("click", PF.obj.listing.selectors.list_item + " a", function (e) {
...
PF.fn.modal = {
    ...
    call: function (options) {
        ...
        var modal_base_template = [
            ...
            '">%MODAL_BODY%</div>%MODAL_BUTTONS%<span class="close-modal icon--close fas fa-times" data-action="close-modal"></span></div></div>'
        ].join("");
...
PF.fn.listing.show = function (response, callback) {
    ...
    $content_listing.imagesLoaded(function (i) {
        var $subjects = $(
            PF.obj.listing.selectors.list_item + ":visible",
            $content_listing
        ),
            $targets = $(
